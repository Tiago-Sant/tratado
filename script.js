...
praysListHeader.forEach(pray => {
    pray.addEventListener("click", function() {
        ...
        localStorage.setItem("data-class", classPray)
    })
})

buttonTheme.addEventListener("click", ()=> {
    pageWrapper.classList.toggle("dark")
    pageWrapper.classList.contains("dark") ? localStorage.setItem("theme-dark","1") : localStorage.removeItem("theme-dark")
})
...
function activeLastPrays() {
    const lastPrayerClass = localStorage.getItem("data-class")

    lastPrayerClass && activeClassHandle(lastPrayerClass)
}

function activeLastTheme() {
    const lastThemeClass = localStorage.getItem("theme-dark")

    lastThemeClass && pageWrapper.classList.add("dark")
}
